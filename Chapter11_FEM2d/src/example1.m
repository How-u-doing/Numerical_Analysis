...
% where $\Omega=\{(x,y)\ | \ -\frac{1}{2}\leq x \leq 1,\ -1 \leq y \leq 1 \}$,
...
n=4;
for i=1:N
    [dofs(i),L_inf_err(i),L2_err(i),H1_err(i)]=run_example1(n);
    n=n*2;
end
L_inf_convergence_order=zeros(N-1,1);
L2_convergence_order=zeros(N-1,1);
H1_convergence_order=zeros(N-1,1);
for i=1:N-1
    L_inf_convergence_order(i)=log2(L_inf_err(i)/L_inf_err(i+1));
    L2_convergence_order(i)=log2(L2_err(i)/L2_err(i+1));
    H1_convergence_order(i)=log2(H1_err(i)/H1_err(i+1));
end
L_inf_convergence_order
L2_convergence_order
H1_convergence_order
...
ylabel('error [log]');
legend('maximum norm','L^2 norm','rate 2','H^1 semi-norm', ...
    'rate 1','Location','SouthWest');
% title('Convergence Orders');

%=================================================================

function [dofs,L_inf_err,L2_err,H1_err] = run_example1(n)
x=[-1/2 1]; y=[-1 1];
% n_x=50; n_y=50;
n_x=n; n_y=n;
...
[L_inf_err,L2_err,H1_err] = errorEstimates(p,t,u,u_r,grad_u_r);
